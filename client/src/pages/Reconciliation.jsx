...
const Reconciliation = () => {
  ...
  const { runs, runsLoading, selectedRunId, selectedRun, setSelectedRunId } = useReconciliation();
  ...
  // If the user has runs but no run is selected, default to the most recent run.
  // Never default to the demo dataset on this page.
  useEffect(() => {
    if (!selectedRunId && runs.length > 0 && !runsLoading) {
      setSelectedRunId(runs[0].id || runs[0]._id);
    }
  }, [selectedRunId, runs, runsLoading, setSelectedRunId]);
  ...
  const loadSummary = useCallback(async () => {
    ...
    try {
      const [runRes, anRes] = await Promise.all([
        recApi.getRun(selectedRunId),
        recApi.getAnalytics(selectedRunId).catch(() => null),
      ]);
      const run = runRes.data?.data;
      const analytics = anRes ? anRes.data?.data : null;
      setSummary({
        run,
        processingTimeMs: run?.processingTimeMs ?? null,
      ...
      setSummary(null);
      setSummaryError(err.response?.data?.error?.message || 'Unable to load reconciliation data. Please try again.');
    } finally {
      setSummaryLoading(false);
    }
  ...
  const loadResults = useCallback(async () => {
    ...
    try {
      const res = await recApi.getResults(selectedRunId, {
      ...
      if (err.response?.status === 401) return;
      setResults([]);
      setResultsError(err.response?.data?.error?.message || 'Unable to load reconciliation results.');
    }
  }, [selectedRunId, runs.length, statusFilter, typeFilter, search, page, pageSize]);
  ...
  const exceptionDistribution = useMemo(() => {
    const out = {};
    for (const [type, info] of Object.entries(summary?.breakdown || {})) {
      out[type] = typeof info === 'object' && info !== null ? info.count ?? 0 : info;
    }
    return out;
  }, [summary]);
  ...
  const typeOptions = Object.keys(exceptionDistribution);
  const maxTypeCount = Math.max(1, ...Object.values(exceptionDistribution).map(Number));
  ...
  const showEmpty = !runsLoading && runs.length === 0;

  if (runsLoading || (runs.length > 0 && summaryLoading)) {
    return (
      <div>
        <div className="mb-6">
    ...
    return (
      <div>
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-12 text-center">
          ...
          <h2 className="text-lg font-medium text-gray-900 mb-2">No reconciliation runs yet.</h2>
          <p className="text-sm text-gray-500 max-w-md mx-auto mb-6">
            Run a reconciliation to see your reconciliation dashboard and financial statistics.
          </p>
          <button
      ...
      <div>
        <div className="flex items-start justify-between mb-6 flex-wrap gap-4">
          ...
          <div className="flex items-center gap-2">
            <RunSelector allowDemo={false} label="Run" />
            <button
              onClick={() => navigate('/reconciliation/runs/new')}
              className="inline-flex items-center gap-2 px-4 py-2.5 bg-primary-600 text-white text-sm font-medium rounded-lg hover:bg-primary-700 transition-colors"
            >
        ...
        <div className="bg-white rounded-xl border border-red-200 p-8 mb-6">
          <div className="text-center py-6">
            ...
            <h2 className="text-base font-semibold text-gray-900 mb-1">Unable to load reconciliation</h2>
            <p className="text-sm text-gray-500 mb-5">{summaryError}</p>
            <button
              onClick={loadSummary}
    ...
    <div>
      ...
      {selectedRun && (
        <div className="mb-6 flex items-center gap-2 text-xs text-gray-500">
          ...
          <span>
            ...
            <span className="font-medium text-gray-700">{selectedRun.name}</span>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
        ...
        <MetricCard
          ...
          value={overview.totalBankTransactions?.toLocaleString('en-IN') ?? '—'}
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-4 mb-6">
        <MetricCard
          icon={CheckCircle2}
          label="Matched"
          value={overview.matchedRecords?.toLocaleString('en-IN') ?? '—'}
        ...
        <MetricCard
          icon={AlertCircle}
          label="Exceptions"
          value={overview.totalExceptions?.toLocaleString('en-IN') ?? '—'}
        ...
        <MetricCard
          ...
          value={formatPercent(matching.paymentMatchRate)}
        ...
        <MetricCard
          ...
          value={formatPercent(matching.bankMatchRate)}
        ...
        <MetricCard
          ...
          value={formatPercent(matching.invoiceMatchRate)}
      ...
      <div className="mb-6">
        <div className="flex items-center justify-between mb-3">
          ...
          <span className="text-xs text-gray-400">{typeOptions.length} types</span>
        </div>
                      ...
                      <div className="flex items-center gap-3">
                        <div className="flex-1 h-2 bg-gray-100 rounded-full overflow-hidden">
                          <div
                            ...
                            style={{ width: `${(exceptionDistribution[type] / maxTypeCount) * 100}%` }}
                          />
                        </div>
                      </div>
                    </td>
      ...
      <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-200">
          <div className="flex items-center justify-between mb-4 flex-wrap gap-3">
            ...
            <span className="text-xs text-gray-400">{totalResults} records</span>
          </div>
